...
class tSNE {
     ...
     */
    public function initDataDist($D) {
        $N = count($D);
        $this->assert($N>0, " X is empty? You must have some data!");
     ...
     * @access private
     */
    private function assert($condition, $message="Assertion failed", $isFatal=true) {
        if( !$condition ) {
     ...
     * @access private
     */
    private function getopt($opt, $field, $dftVal) {
     ...
     * @access private
     */
    private function randn2d($n, $d, $s=null) {
        $x = array();
        for($i=0; $i<$n; $i++) {
            $xhere = array();
            for($j=0; $j<$d; $j++) {
     ...
     * @access private
     */
    private function d2p($D, $perplexity, $tol) {
